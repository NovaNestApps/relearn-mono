# relearn-mobile — Codex / Agent Instructions
...
1. Create `lib/features/my_feature/data/my_feature_repository.dart`
   ...
   - Use resilient null-safe JSON parsing
...
- Pass complex objects via `extra: { 'key': value }` map

## Implementation Notes

- Null-safe JSON: `json['field'] as String? ?? ''`
- Numbers: `(json['value'] as num?)?.toDouble() ?? 0.0`
- Lists: `(json['list'] as List? ?? []).whereType<Map>().map(...)...`
- After API calls: `if (mounted) setState(...)`
